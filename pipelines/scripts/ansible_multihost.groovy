...
def setupParams(){
    ...
    jobParams = [
        choice(name: 'PLAY_BOOK', choices: ['whoami-playbook.yml','jvm_restart.yml'], description: 'Playbook to Execute')
    ]
...
def executeJob() {
    ...
    ansible_helper.execute_simple_playbook_on_host("/application/ansible/inventory/${ENV_TYPE}/${CLUSTER_NAME}/${Application}","${SERVER}","${WORKSPACE}/ansible/${PLAY_BOOK}")
}
